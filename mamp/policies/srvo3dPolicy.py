...
@ Author: Gang Xu
@ Date: 2022.04.16
@ Details: RVO3D+Dubins for multi-agent motion planning
...
from mamp.util import sqr, absSq, l3norm, is_intersect, satisfied_constraint, get_phi, reached, cartesian2spherical


class SRVO3DPolicy(object):
    """ SRVO3DPolicy """
    ...
    def find_next_action(self, dict_comm, agent, kdTree):
        """
        Function: SRVO3DPolicy compute suitable speed for agents
        """
        start_t = time.time()
...
def shunted_strategy(agent, v_list):
    ...
    i = 1
    while opti_flag:
        diff = abs(l3norm(v_list[0], vA) - l3norm(v_list[i], vA))
        if diff < 1e-1:
